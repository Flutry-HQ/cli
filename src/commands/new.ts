...
export async function promptQuestions(): Promise<Answers> {
  ...
  const database = await confirm({ message: `${chalk.yellow('🗄️')} Enable database?`, default: false });
  const databaseAnswers = database
    ? {
        dbName: await input({ message: `${chalk.yellow('◈')} Database name:`, default: 'flutrydb' }),
        dbUser: await input({ message: `${chalk.yellow('◈')} Database user:`, default: 'root' }),
        dbPass: await password({ message: `${chalk.yellow('◈')} Database password:`, mask: '*' }),
        dbHost: await input({ message: `${chalk.yellow('◈')} Database host:`, default: 'localhost' }),
        dbPort: await input({ message: `${chalk.yellow('◈')} Database port:`, default: DEFAULTS.dbPort, validate: validatePort }),
        dbType: await select({
  ...
  return {
    ...
    ...databaseAnswers,
    packageManager,
    installNow: await confirm({ message: `${chalk.green('⚡')} Install dependencies now?`, default: true }),
  };
}
...
async function createProject(answers: Answers): Promise<string> {
  ...
  const spinner = ora({ text: chalk.cyan('Creating project'), spinner: 'dots12', color: 'cyan' }).start();
...
async function installPackages(targetDir: string, packageManager: PackageManager): Promise<void> {
  const spinner = ora({ text: chalk.yellow(`Installing dependencies with ${packageManager}`), spinner: 'arc', color: 'yellow' }).start();
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const child = spawn(command, commandArgs, { cwd: targetDir, stdio: ['ignore', 'pipe', 'pipe'] });
    let errorOutput = '';
    ...
    child.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`${packageManager} install failed${errorOutput ? `: ${errorOutput.trim()}` : ''}`));
    });
  });
...
export default async function newCommand(): Promise<void> {
  try {
    ...
    if (answers.installNow) await installPackages(targetDir, answers.packageManager);
    ...
    if (!answers.installNow) console.log(`${chalk.greenBright('│')}   ${chalk.yellow(`${answers.packageManager} install`)}`);
